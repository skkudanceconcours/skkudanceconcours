...
const FilePreview = ({ files }: FilePreviewProps): ReactNode => {
  const fileDisplay = files.map(file => (
    <div
      key={file.uuid}
      className="flex  h-fit w-full items-center justify-start overflow-hidden text-ellipsis whitespace-nowrap py-2 text-sm hover:cursor-pointer hover:text-blue-400"
