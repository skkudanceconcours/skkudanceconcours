...
const NextTable2024 = ({ receptions, onDelete, onUpdate, readonly }: TableProps): ReactNode => {
  ...
  const handleBulkDelete = async () => {
    ...
    await Promise.all(ids.map(id => deleteReception("2024", id)));
    onDelete(ids);
  ...
  const renderCell = useCallback(
    (reception: Reception2024 & { _selected: boolean }, columnKey: Columnkey) => {
      switch (columnKey) {
        case "select":
          return (
            <input
